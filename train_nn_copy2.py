...
for nr_neurons in range(2, 30):
    ...
    if nr_digits == 2:
        ...
        y_test = np.where(y_test == 5, 0, 1)      # Same mapping for test set
    ...
    class PredictionCallback(Callback):
        ...
        # np.save(f'training_digits_{try_nr}\\predictions.npy', self.predictions_batch)
            ...
            for i in [0,1]:#range(len(self.model.layers)):
            #i=2
    ...
    prediction_callback = PredictionCallback(x_train_gray, high_dim_points_normalized, len(model.layers))
    ...
    if FIT_MODEL:
        ...
        model.save(f'trainings\\training_digits_{try_nr}\\model_filename_{nr_digits}.h5')
    ...
    #random_points = np.random.uniform(-5, 5 ,  (400000, x_train_gray.shape[1]))

    #color_space_labels = "rgb(200, 200, 200)"
    #random_points = model_1.predict(random_points)

    #random_points = np.random.uniform(-1, 1 ,  (100000, x_train_gray.shape[1]))
    ...
    #x_train_gray = random_points
    #y_train_mapped = color_space_labels
    ...
    random_points = np.random.uniform(-20, 20 ,  (100000, x_train_gray.shape[1]))
    color_space_labels = model_2.predict(random_points).argmax(axis = 1)
    color_space_labels = "gray"
