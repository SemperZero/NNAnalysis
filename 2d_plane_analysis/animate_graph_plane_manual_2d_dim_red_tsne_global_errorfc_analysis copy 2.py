...
#    # X_pred_space = np.load(f'trainings2\\training_digits_{try_nr}\\space_bent_output_layers_{predict_layer}.npy')
    ...
    X_latent_train = np.load(f'trainings2/training_digits_{try_nr}/space_bent_output_layers_{selected_layer}.npy')
    X_pred_train = np.load(f'trainings2\\training_digits_{try_nr}\\space_bent_output_layers_{predict_layer}.npy')
   ...
   # X_latent_space = relu(X_latent_space)
    ...
    pca = PCA(n_components=2).fit(relu(X_latent_train[-1]))
    ...
    axis_ranges = get_axis_ranges(X_latent_train)
    ...
    #TODO: argmax on latent layer what does that mean? did i mistakenly make it? reproduce

    frame_nr = 0
    for ii in range(0, len(X_latent_train), 1):

        X_latent_train_ii = X_latent_train[ii]
        z2 = pca.transform(relu(X_latent_train_ii))
        
        clusters, labels = quadrant_clusters(X_latent_train_ii, 0.0)

        # assign color on first encounter
        for lbl in labels:
            if lbl not in label_to_color:
                label_to_color[lbl] = mcolors.to_hex(rng.random(3))

        proj3d_mesh = pca.transform(X_latent_train_ii)
        point_colors = [label_to_color[lbl] for lbl in labels]


        pred_labels = np.argmax(X_pred_train[ii], axis = 1)
      ...
      #  y_pred_train = softmax(z2, axis = -1)
       ...
       # vectors_update = -dL_dZ
        ...
        fig_layer.add_trace(go.Scatter(
            ...
            y=z2[:,1], 
            mode='markers',
            marker=dict(size=7, color=point_colors, opacity=0.5),
            showlegend=False
        ))
