...
def plot_training_spacebent_all_smooth():
    ...
    # load latent states
    X_latent = np.load(f'trainings2/training_digits_{try_nr}/space_bent_output_layers_{selected_layer}.npy')
    X_pred = np.load(f'trainings2\\training_digits_{try_nr}\\space_bent_output_layers_{predict_layer}.npy')
   ...
   # exit()
    ...
    for ii in range(first_frame, first_frame + nr_global_frames, 1):
        ...
        #     y=V_flat, 
        #     mode='markers',
        #     marker=dict(size=6, color=pred_labels, colorscale = [[0, "red"], [0.5, "blue"], [1, "green"]], opacity=0.5),
        #     showlegend=False
        # ))
